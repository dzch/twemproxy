...
int
range_dispatch(struct server_pool *pool, struct continuum *continuum, uint32_t ncontinuum, uint32_t hash, int op_write)
{
	...
	if (c == NULL) {
        log_error("range_dispatch: failed to fetch server");
        c = array_get(p, random() % array_n(p));
	}
