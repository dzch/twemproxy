...
conn_sendv(struct conn *conn, struct array *sendv, size_t nsend)
{
    ...
    for (;;) {
        ...
        if (errno == EINTR) {
            ...
            conn->err = errno;
			if (conn->client) {
                log_error("sendv on sd %d failed: %s, peer is client", conn->sd, strerror(errno));
			} else {
                log_error("sendv on sd %d failed: %s, peer is server: %s", conn->sd, strerror(errno),(conn->owner) ? ((struct server*)(conn->owner))->pname.data : "");
			}
            return NC_ERROR;
        }
    }
